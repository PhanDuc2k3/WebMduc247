  ...
  exports.sendMessage = async (req, res) => {
    try {
      ...
      for (const file of filesArray) {
        if (file.path) {
          // nếu multer lưu local hoặc dùng đường dẫn Cloudinary trước đó
      ...
      try {
        await axios.post(`${SOCKET_SERVICE_URL}/api/socket/emit`, {
          event: "new_message",
          data: message,
          room: conversationId, // gửi theo conversationId, không phải all
        });
        console.log(`📡 Event emitted: new_message -> room: ${conversationId}`);
      } catch (socketErr) {
