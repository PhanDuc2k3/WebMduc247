...
const CategoryProducts: React.FC<Props> = ({
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-[#4B5563]/30 p-4 sm:p-6 lg:p-8 animate-fade-in-up delay-200">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
        <div className="flex-1 min-w-0">
          <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#4B5563] mb-1">
      ...
      {categoryProducts.length === 0 ? (
        <div className="text-center py-8 sm:py-12 bg-[#4B5563]/5 rounded-lg sm:rounded-xl border-2 border-dashed border-[#4B5563]/30 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-6">
          {categoryProducts.map((prod, index) => (
            <div
              key={prod._id}
              className="p-3 sm:p-4 bg-gradient-to-br from-white to-gray-50 rounded-lg sm:rounded-xl border-2 border-gray-200 hover:border-[#4B5563] hover:shadow-lg transition-all duration-300 animate-fade-in-up flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0"
      ...
      {/* Add Products Button */}
      <button
        className="w-full px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-[#4B5563] text-white rounded-lg sm:rounded-xl font-bold shadow-lg hover:shadow-xl active:scale-95 sm:hover:scale-105 hover:bg-[#374151] transition-all duration-300 touch-manipulation"
      ...
      {showProductModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] sm:max-h-[80vh] overflow-hidden animate-scale-in flex flex-col">
            <div className="p-4 sm:p-6 border-b-2 border-[#4B5563]/30 flex-shrink-0 bg-[#4B5563]/5">
              <div className="flex items-start sm:items-center justify-between gap-2">
                <div className="flex-1 min-w-0">
                  <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-[#4B5563]">Chọn sản phẩm để thêm</h2>
                      ...
                      <label
                        ...
                        className={`flex items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-lg sm:rounded-xl border-2 cursor-pointer transition-all duration-300 touch-manipulation ${
                          disabled
                            ...
                            : isSelected
                            ? 'bg-[#4B5563]/5 border-[#4B5563] shadow-md'
                            : 'bg-white border-gray-200 active:border-[#4B5563]/60 active:bg-[#4B5563]/5'
                        }`}
                      >
                        <input
                          type="checkbox"
                          ...
                          disabled={disabled}
                          className="w-4 h-4 sm:w-5 sm:h-5 text-[#4B5563] rounded focus:ring-2 focus:ring-[#4B5563] cursor-pointer flex-shrink-0"
                        />
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-1 sm:gap-2 mt-1">
                            ...
                            {disabled && (
                              <span className="text-xs text-[#4B5563] font-semibold">(Đã có trong danh mục)</span>
                            )}
                          </div>
                        </div>
                      </label>
            ...
            <div className="p-4 sm:p-6 border-t-2 border-[#4B5563]/30 flex flex-col sm:flex-row justify-end gap-2 sm:gap-3 flex-shrink-0 bg-gray-50">
              <button
              ...
              <button
                className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 text-sm sm:text-base bg-[#4B5563] text-white rounded-lg sm:rounded-xl font-bold shadow-lg hover:shadow-xl active:scale-95 sm:hover:scale-105 hover:bg-[#374151] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation"
