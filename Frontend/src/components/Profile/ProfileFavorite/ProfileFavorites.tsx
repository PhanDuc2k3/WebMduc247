    ...
    return (
      <div className="bg-white rounded-xl md:rounded-2xl shadow p-4 md:p-6 lg:p-8">
        <div className="text-center py-8 md:py-12 lg:py-16">
          <Clock size={40} className="mx-auto mb-3 md:mb-4 animate-pulse text-[#4B5563]" />
      ...
      <div className="flex gap-2 md:gap-4 mb-4 md:mb-6 border-b border-gray-200 pb-2">
        <button
          ...
          className={`pb-2 md:pb-3 px-3 md:px-4 font-bold transition-all duration-300 relative flex items-center gap-1.5 md:gap-2 text-xs sm:text-sm md:text-base ${
            activeTab === "products"
              ? "text-[#4B5563]"
              : "text-gray-500 hover:text-[#4B5563]"
          }`}
        >
          <ShoppingBag size={16} className="md:w-[18px] md:h-[18px] text-[#4B5563]" />
          Sản phẩm ({products.length})
          {activeTab === "products" && (
            <span className="absolute bottom-0 left-0 right-0 h-0.5 md:h-1 bg-[#4B5563] rounded-full"></span>
          )}
        </button>
        <button
        ...
        >
          <Store size={16} className="md:w-[18px] md:h-[18px] text-[#4B5563]" />
      ...
      {activeTab === "products" && (
        <div>
          {products.length > 0 ? (
            ...
            <div className="text-center py-8 md:py-12 lg:py-16">
              <Heart size={48} className="mx-auto mb-3 md:mb-4 text-[#4B5563]" />
      ...
      {activeTab === "stores" && (
        <div>
          {stores.length > 0 ? (
            ...
            <div className="text-center py-8 md:py-12 lg:py-16">
              <Store size={48} className="mx-auto mb-3 md:mb-4 text-[#4B5563]" />
              <p className="text-gray-500 text-sm sm:text-base md:text-lg font-medium mb-2">
